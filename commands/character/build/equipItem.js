exports.subcommands = ["equipitem", "equipItem"];
exports.run = async (client, message, args, level, charactername) => {
    ...
    if (args.length < 3) {
        message.author.send(
            "🛑🛑 **ERROR** Kupopo!? **ERROR** 🛑🛑\n" + 
            `Unable to find the item/slot you entered.`);
        return;
    } else {
        args.shift();
        slot = args[0];
    ...
    await client.db[authorId].get("characters").find({name:charactername}).get("equipInventory").find({id:itemId}).assign({slot:slot}).write();
...
Item '${itemName}' equipped to ${slot} slot. Current character's inventory:
